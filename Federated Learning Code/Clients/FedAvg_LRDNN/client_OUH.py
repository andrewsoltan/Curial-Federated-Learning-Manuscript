...
siteName = "OUH"
trainingOnly = False;
pathToAllData = "../raw/CURIAL-FL OUH Prepandemic through 06 Mar 2021.csv"
trainingCutOffDate = '2020-10-29'
prepandemicPeriodStartDate = '2018-12-01'
...
if __name__ == "__main__":
    ...
    fullDataset = preprocessor.preprocess(pathToAllData)

    ##############
    #Stage 2: Formation of correct training & validation cohorts (where applicable), noramlisation & imputation
    #Enforce a pre-pandemic control cohort start Date
    #print ("Full length" + str(fullDataset.shape[0]))
    preprocessedTrainingDataset = fullDataset[fullDataset.ArrivalDateTime >= prepandemicPeriodStartDate]
    #print ("Truncating up to Dec 2019 length" + str(preprocessedTrainingDataset.shape[0]))

    #Generate the Training set with reference to Cut Off Date for training
    preprocessedTrainingDataset = preprocessedTrainingDataset[preprocessedTrainingDataset.ArrivalDateTime <= trainingCutOffDate]
    ...
    while relaunchOnCompletion == True:
        ...
        class LRClient(fl.client.NumPyClient):
            ...
            def fit(self, parameters, config):  # type: ignore
                ...
                #Save an example of the transmitted model weights
                #print (utils.get_model_parameters(model))
                #pd.DataFrame(utils.get_model_parameters(model)[0]).to_csv("Example LR model weights.csv")
            ...
            def evaluate(self, parameters, config):  # type: ignore
                ...
                #Save an example of the transmitted model results
                #TestSetResults.loc[self.iteration,:].to_csv("Example LR Results row.csv")
        ...
        class DNNClient(fl.client.NumPyClient):
            ...
            def fit(self, parameters, config):
                ...
                #pd.DataFrame(model.get_weights()).to_csv("Example DNN model weights.csv")
